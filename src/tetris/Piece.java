...
public class Piece {
	...
	private int calcWidth(TPoint[] points) {
		
		int mn = points[0].x, mx = points[0].x;
		for (int i=1; i<points.length; i++) {
			
			mn = min(mn, points[i].x);
	...
	private int calcHeight(TPoint[] points) {
		
		int mn = points[0].y, mx = points[0].y;
		for (int i=1; i<points.length; i++) {
		
			mn = min(mn, points[i].y);
	...
	private int[] calcSkirt(TPoint[] points) {
	...
	*/
	public Piece(TPoint[] points) {
		this.body = points;
		this.width = this.calcWidth(points);
		this.height = this.calcHeight(points);
		this.next = null;
		this.skirt = this.calcSkirt(points);
	}
	...
	*/
	...
	private TPoint[] calcRotation() {
		
		TPoint[] newBody = new TPoint[body.length];
		int x, y;
		for (int i=0; i<body.length; i++) {
			x = 1 - (body[i].y - (height - 2)); // es online vnaxe imedia ar damdzraxavt :)
			y = body[i].x;
	...
	public Piece computeNextRotation() {
		next = new Piece(this.calcRotation());
		return next;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		// standard equals() technique 3
		if (this.body.length != other.body.length) return false;
		
		// standard equals() technique 4
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		Piece previous = root;
		while (true) {
			Piece next = previous.computeNextRotation();
			previous.next = next;
			if (root.equals(next)) {
				return root;
			}
